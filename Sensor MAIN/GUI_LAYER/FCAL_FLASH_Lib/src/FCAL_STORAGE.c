...
//
//uint16_t CAL_TYPE;
//uint16_t g_buf_cal[3],a_buf_cal[12],m_buf_cal[6],q_buf_cal[16] ;
...
//  for(i=2;i<34;i+=2)
//  {
//   memcpy(&q_buf_cal[n],(char *)(FLASH_ADD_Q+i), 2);
//   Q_COEF[n]=((double)((s16)(q_buf_cal[n])));
//   n++;
//  } 
//  uint16_t res[1]; 
//  memcpy(&res[0],(char *)(FLASH_ADD_Q+i), 2);
//  double res_double= (double)(res[0]);
//  for(i=0;i<16;i++)
//  {
//    Q_COEF[i]=Q_COEF[i]/(pow(10, res_double));
//  }
//}
///*----------------------------------------------------------------------------*/
...
//{
//  for(int i=0;i<16;i++)
//  q_cal[i]=((uint16_t)(coeff_cal[2*i+1]<<8)+coeff_cal[2*i]);
//  /*----------------FLASH_STORAGE--------------*/
//  FLASH_Unlock(); 
//  int n=0; 
//  if (FLASH_ErasePage(FLASH_ADD_Q) == FLASH_COMPLETE)
//  {
//    STATUS=FLASH_ProgramHalfWord(FLASH_ADD_Q, 1);
//    if(STATUS==!FLASH_COMPLETE) while(1){}
//    int i=0;  
//    for(i=2;i<34;i+=2)
//    {
//      STATUS=FLASH_ProgramHalfWord(FLASH_ADD_Q+i, q_cal[n]);
//      n++;
//      if(STATUS==!FLASH_COMPLETE)while(1){}
//      
//    }
//    uint16_t res= (uint16_t)(exp);
//    STATUS=FLASH_ProgramHalfWord(FLASH_ADD_Q+i, res);
//    if(STATUS==!FLASH_COMPLETE)while(1){}
//  }
//  FLASH_Lock();
//  NVIC_SystemReset();
...
//{
//    int i=5; u8 pow=0; u8 q_cal_f[32];
//    pow=UART_IN[index-i]; index--;
//    for(int n=31;n>=0;n--){q_cal_f[n]=UART_IN[index-i];i++;}
//    Q_Cal_Storage(q_cal_f, pow);
//}
///*----------------------------------------------------------------------------*/
//void GYRO_NEW_CAL(char UART_IN[100], int index)
//{
//    int i=5; u8 gyro_cal_f[6];
//    for(int n=5;n>=0;n--){gyro_cal_f[n]=UART_IN[index-i];i++;}
//    G_CAL_COEFF=NEW_CAL; GYRO_Cal_Storage(gyro_cal_f);
//}
///*----------------------------------------------------------------------------*/
//void ACC_NEW_CAL(char UART_IN[100], int index)
//{
//    int i=5; u8 acc_cal_f[24];
//    for(int n=23;n>=0;n--){acc_cal_f[n]=UART_IN[index-i];i++;}
//    A_CAL_COEFF=NEW_CAL;ACC_Cal_Storage(acc_cal_f);
//}
///*----------------------------------------------------------------------------*/
//void MAG_NEW_CAL(char UART_IN[100], int index)
//{
//    int i=5; u8 mag_cal_f[12];
//    for(int n=11;n>=0;n--){mag_cal_f[n]=UART_IN[index-i];i++;}
//    M_CAL_COEFF=NEW_CAL; MAG_Cal_Storage(mag_cal_f);
//}
///*----------------------------------------------------------------------------*/
...
//{
//  memcpy(&CAL_TYPE,(char *)(FLASH_ADD_Q), 2);
//  if(CAL_TYPE==1)
//  {
//    Q_FLASH_READ();
//    ASSIGN_Q_MATRIX(Q_COEF);
//  }
//  else
//  {
//    Q_COEF[0]= 0.1e-005;Q_COEF[1]=-0.1e-005;Q_COEF[2]=-0.1e-005;Q_COEF[3]=-0.1e-005;
//    Q_COEF[4]=-0.1e-005;Q_COEF[5]= 0.1e-005;Q_COEF[6]=-0.1e-005;Q_COEF[7]=-0.1e-005;
//    Q_COEF[4]=-0.1e-005;Q_COEF[5]=-0.1e-005;Q_COEF[6]= 0.1e-005;Q_COEF[7]=-0.1e-005;
//    Q_COEF[4]=-0.1e-005;Q_COEF[5]=-0.1e-005;Q_COEF[6]=-0.1e-005;Q_COEF[7]= 0.1e-005;
//    ASSIGN_Q_MATRIX(Q_COEF);
//  }
